...
# TODO:There seems to be something wrong with the code below
# rm -rf ./build-gcc
# rm -rf ./build-binutils
# rm -rf ./wrs-vxworks-headers
# rm -rf ./install
# rm -rf ./gccdist


# Extract VxWorks 6.3 headers 
...
# Extract VxWorks 6.9 headers 
# TODO: Fix VxWorks 6.9 headers
# unzip -o vxworks69gccdist.zip
# mkdir -p wrs-vxworks-headers/{wind_base/target,share/ldscripts}
# cp -dpr --no-preserve=ownership gccdist/WindRiver/vxworks-6.9/host wrs-vxworks-headers/wind_base
# cp -dpr --no-preserve=ownership gccdist/WindRiver/vxworks-6.9/target/h/. wrs-vxworks-headers/sys-include
# cp -dpr --no-preserve=ownership gccdist/WindRiver/vxworks-6.9/target/h/wrn/coreip/. wrs-vxworks-headers/include
# cp -dpr --no-preserve=ownership gccdist/WindRiver/vxworks-6.9/target/h/wrn/coreip/. wrs-vxworks-headers/wind_base/target/h
# export WIND_BASE=${SCRIPTPATH}/wrs-vxworks-headers/wind_base
# export VSB_DIR=""
...
# Extract everything
for f in *.tar*; do tar -xaf $f; done
...
mkdir -p $INSTALL_PATH/include/i586-wrs-vxworks/cross-include/sys
cp ${SCRIPTPATH}/sys_time.h $INSTALL_PATH/include/i586-wrs-vxworks/cross-include/sys/time.h
# We also add a dummy select.h that just includes VxWorks's selectLib.h 
cp ${SCRIPTPATH}/sys_select.h $INSTALL_PATH/include/i586-wrs-vxworks/cross-include/sys/select.h

# Also the poll.h and the uio.h header are not in the correct location, we copy it there 
cp ${SCRIPTPATH}/wrs-vxworks-headers/sys-include/wrn/coreip/poll.h $INSTALL_PATH/include/i586-wrs-vxworks/cross-include/sys/poll.h
cp ${SCRIPTPATH}/wrs-vxworks-headers/include/net/uio.h $INSTALL_PATH/include/i586-wrs-vxworks/cross-include/sys/uio.h


# Copy the custom CMake toolchain file 
cp ./vxworks_toolchain.cmake.in $INSTALL_PATH/vxworks_toolchain.cmake
